"""
An ppplication for RS package:
Conduct Monte Carlo experimentsl to quantify uncertainties pertaining to
multiaxial flow stress measurements.

ex01:
combination of N(psi) and N(F). In each cell,
a multiple number of error-sigma is given.

ex02:
combination of N(psi) and tilting range (max psi).
In each cell, a multiple number of error-sigma is
given
"""
...
def main_ex02(
        ## main arguments (2d plane)
        N_psi    = [7,11,15,29],
        max_psis = [15,30,45,90], ## y-axis


    ## secondary arguments
    ensemble = 10,
    N_F      = 3,
    sigmas   = [1e-5,1e-4,2e-4],
    ##
    isub     = True,
    wait_sec = 13.,
    min_cpu  = 48,
    ...
    max_s2p = fpsis(*max_psis)
    if wait_sec>10 and min_cpu<10:
        print 'Consider reducing the wait sec'
        time.sleep(2)

    if isub: import os
    njobs = len(N_psi) * len(max_psis)
    cores, nj, que_kinds = fa(
        njobs=njobs,allow_fq=True,min_cpu=min_cpu)

    if cores==0:
        print 'No ques are available with'+\
            ' the given min_cpu==%i'%min_cpu
        return
    if len(cores)<njobs: print 'deficiency '+\
            'in available ques'
    ## write a bash script and submit
    bash_fns=[]
    for i in range(len(max_psis)): ## x
        for j in range(len(N_psi)): ## y
            ## renew cores
            que_kinds, use_cpu = renew_resource(
                njobs, True, min_cpu, max_cpu)
            ## end of renew
            ### Begin sh scripting
            fn = 'rs_par_ex02_%1i_%1i.sh'%(i+1,j+1)
            bash_fns.append(fn)
            f = open(fn,'w')
            write_bash_rs(
                f,i,j,noti,que_kinds,use_cpu,
...
def main_ex03(
    ...
    ## sorting sigmas according to the ascending order
    sigmas.sort()
    ...
    for i in range(len(N_F)):
        for j in range(len(max_psis)):
            ...
            bash_fns.append(fn)
            f = open(fn,'w')
            sin2psi = sin(max_psis[j]*pi/180.)**2
            write_bash_rs(
                f,i,j,noti,que_kinds,use_cpu,
...
def plot_ex02():
    """
    Variables: N(F):x axis; and N(Psi):y axis
    """
    from jac_par_ex1 import plot as reader
    import matplotlib.pyplot as plt
    import matplotlib.gridspec as gridspec
    ...
    nx=int(fns[-1].split('.dat')[0][::-1][0])
    ny=int(fns[-1].split('.dat')[0][::-1][2])

    gs=gridspec.GridSpec(
        nx,ny,wspace=0.0,hspace=0.0,left=0.22,
        right=0.8,top=0.80)
    ls=['^','s','o','d','+']
    fc=['k','k','k','k','k']## fc=['r','g','b','m','y']

    for i in xrange(len(fns)):
        ix=int(fns[i].split('.dat')[0][::-1][0])
        iy=int(fns[i].split('.dat')[0][::-1][2])
        ...
        try:
            ...
            ss     = dicts['ss']
            nbins  = dicts['nbins']
            max_s2p= dicts['bounds'][1]
            max_psi = fx(max_s2p)

            ax = plt.subplot(gs[ix-1,iy-1])

            ## ax.set_title('(%1i,%1i)'%(ix,iy),
            ##              dict(fontsize=8))
            if ix==nx and iy == 1: ax_p = ax
            ...
            for j in xrange(len(sigmas)):
                ...
                if ix==1:
                    ax.plot(
                        dat[0][::ss],(mu+sigma1)[::ss],
                        ls[j],mfc='None',color='k',
                        label=r'%3i $\mu$ strain'%(
                            sigmas[j]*1e6))
                ax.plot(dat[0], mu+sigma1,'-',
                        mfc='None',
                        color='k')
                ax.plot(dat[0], mu-sigma1,'-',
                        mfc='None',
            ...
            if ix==1 and iy==ny:
                ax.legend(loc='best',ncol=1,
                          ...
                          numpoints=1,prop={'size':7})

            if ix==1:
                ax.annotate(
                    s=r'Max($\sin^2{\psi})=%3.2f$'%max_s2p,
                    size=7,
                    horizontalalignment='center',
                    xy=(0.5,1.6),
                    xycoords='axes fraction')
                ax.annotate(
                    s=r'Max($\psi)=%3.2f^\circ$'%max_psi,
                    size=7,
                    horizontalalignment='center',
                    xy=(0.5,1.25),
                    xycoords='axes fraction')
    ...
    mpl_lib.ticks_bins_ax_u(fig.axes,3)
    try:
        ax_p.set_xticks(np.linspace(0.,1,3))
        #ax_p.set_yticks(np.linspace(0.,0.25,6))
        plt.setp(ax_p.get_xticklabels(),visible=True)
        plt.setp(ax_p.get_yticklabels(),visible=True)
        axes_label.__deco__(iopt=9,ft=13,ax=ax_p)
    except: pass
    ## annotate to the figure canvas
    # plt.annotate(s=r'Maximum tilting angle',
    #              horizontalalignment='center',
    #              size=15,xy=(0.6,0.93),
    #              xycoords='figure fraction',
    #              )
    # plt.annotate(s=r'$N^\psi$ (Number of tilting angles)',
    #              verticalalignment='center',
    #              rotation='vertical',
    #              size=15,xy=(0.02,0.5),xycoords='figure fraction')
    fig.savefig('max_Npsi.pdf')

def plot_ex03():
    """
    Variables: max_psis, N_F; y axis
    """
    from jac_par_ex1 import plot as reader
    import matplotlib.pyplot as plt
    import matplotlib.gridspec as gridspec
    ...
    for i in range(len(fns)):
        ...
        try:
            ...
            s = '%3.2f'%max_s2p
            if ix==nx:
                s = 'Max($\sin^2\psi)=%3.2f$'%max_s2p
            s1 = '$%i^\circ{}$'%max_psi
            if ix==nx:
                s1 = 'Max($\psi)=%i^\circ{}$'%max_psi
